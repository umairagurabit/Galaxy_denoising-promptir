...
class OverlapPatchEmbed(nn.Module):
    def __init__(self, in_c=1, embed_dim=48, bias=False): #in_c was 3
...
class PromptGenBlock(nn.Module):
    def __init__(self,prompt_dim=128,prompt_len=6,prompt_size = 96,lin_dim = 192):
        super(PromptGenBlock,self).__init__()
        # self.prompt_param = nn.Parameter(torch.rand(1,prompt_len,prompt_dim,prompt_size,prompt_size))
        # self.linear_layer = nn.Linear(lin_dim,prompt_len)
        # self.conv3x3 = nn.Conv2d(prompt_dim,prompt_dim,kernel_size=3,stride=1,padding=1,bias=False)
        # self.prompt_param = nn.Parameter(torch.rand(1, prompt_len, prompt_dim, 21, 21))
        # self.linear_layer = nn.Linear(lin_dim, prompt_len)
        self.conv3x3 = nn.Conv2d(prompt_dim, prompt_dim, kernel_size=3, stride=1, padding=1, bias=False)

        self.adjust_channels = nn.Conv2d(3, prompt_dim, kernel_size=1, stride=1, padding=0, bias=False) # change init dim to 3 if using e1e2
    ...
    def forward(self, x, psf_patch, e1e2_patch):
        ...
        # # Reshape PSF to match the prompt size
        # psf_patch = F.interpolate(psf_patch, size=(96, 96), mode="bilinear", align_corners=False)
        print('psf_patch :', psf_patch.shape)

        # Expand e1 and e2 to match the spatial dimensions and batch size
        e1, e2 = e1e2_patch[:, 0], e1e2_patch[:, 1]
        e1 = e1.view(B, 1, 1, 1).expand(B, 1, 21, 21)
        e2 = e2.view(B, 1, 1, 1).expand(B, 1, 21, 21)

        psf_patch = torch.cat((psf_patch, e1, e2), dim=1)
        psf_patch = self.adjust_channels(psf_patch)
        # psf_patch = psf_patch.unsqueeze(1).repeat(1, prompt_weights.size(1), 1, 1, 1)

        # prompt = prompt_weights.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) * psf_patch
        # prompt = torch.sum(prompt,dim=1)
        # prompt = F.interpolate(prompt,(H,W),mode="bilinear")
        # prompt = self.conv3x3(prompt)

        ### direct ###
        psf_patch = self.conv3x3(psf_patch)
        prompt = F.interpolate(psf_patch,(H,W),mode="bilinear")
...
class PromptIR(nn.Module):
    def __init__(self, 
        ...
        if self.decoder:
            self.prompt1 = PromptGenBlock(prompt_dim=64,prompt_len=6,prompt_size = 21,lin_dim = 96)
            self.prompt2 = PromptGenBlock(prompt_dim=128,prompt_len=6,prompt_size = 21,lin_dim = 192)
            self.prompt3 = PromptGenBlock(prompt_dim=320,prompt_len=6,prompt_size = 21,lin_dim = 384)
    ...
    def forward(self, inp_img,psf_patch,e1e2_patch,noise_emb = None):
        ...
        if self.decoder:
            ...
            dec3_param = self.prompt3(latent, psf_patch, e1e2_patch)
        ...
        if self.decoder:
            dec2_param = self.prompt2(out_dec_level3, psf_patch, e1e2_patch)
        ...
        if self.decoder:
            ...
            dec1_param = self.prompt1(out_dec_level2, psf_patch, e1e2_patch)
