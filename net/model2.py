...
class PromptGenBlock(nn.Module):
    def __init__(self,prompt_dim=128,prompt_len=5,prompt_size = 96,lin_dim = 192):
        ...
        self.adjust_channels = nn.Conv2d(1, prompt_dim, kernel_size=1, stride=1, padding=0, bias=False)
        

    def forward(self, x, psf_patch):
        B,C,H,W = x.shape
        emb = x.mean(dim=(-2,-1))
...
class PromptIR(nn.Module):
    def __init__(self, 
        ...
        if self.decoder:
            self.prompt1 = PromptGenBlock(prompt_dim=64,prompt_len=1,prompt_size = 21,lin_dim = 96)
            self.prompt2 = PromptGenBlock(prompt_dim=128,prompt_len=1,prompt_size = 21,lin_dim = 192)
            self.prompt3 = PromptGenBlock(prompt_dim=320,prompt_len=1,prompt_size = 21,lin_dim = 384)
    ...
    def forward(self, inp_img,psf_patch,noise_emb = None):
        ...
        if self.decoder:
            ...
            dec3_param = self.prompt3(latent, psf_patch)
        ...
        if self.decoder:
            dec2_param = self.prompt2(out_dec_level3, psf_patch)
        ...
        if self.decoder:
            ...
            dec1_param = self.prompt1(out_dec_level2, psf_patch)
