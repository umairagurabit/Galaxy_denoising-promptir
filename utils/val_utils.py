...
def compute_psnr_ssim(recoverd, clean):
    ...
    for i in range(recoverd.shape[0]):
        ...
        img_size = min(clean[i].shape[0], clean[i].shape[1])
        win_size = 7 if img_size >= 7 else img_size
        ssim += structural_similarity(clean[i], recoverd[i], data_range=1, channel_axis=-1, win_size=win_size)
