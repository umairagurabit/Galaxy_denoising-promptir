...
class PromptTrainDataset(Dataset):
    def __init__(self, args):
        ...
        # Load PSF files
        self.psf_files = self._load_psf_files()
        self.e1e2_files = self._load_e1e2_files()
    ...
    def _load_psf_files(self):
        psf_files = {}
        psf_dir = self.args.psf_dir
        for file in os.listdir(psf_dir):
            if file.endswith('.npy'):
                psf_id = file.split('.')[0]
                if not os.path.exists(os.path.join(psf_dir, file)) :
                    print("file does not exist psf")
                psf_files[psf_id] = np.load(os.path.join(psf_dir, file))
        # print("PSF files loaded with keys:", list(psf_files.keys()))
        return psf_files
    
    def _load_e1e2_files(self):
        e1e2_files = {}
        e1e2_dir = self.args.e1e2_dir
        print(e1e2_dir)
        for i in range(len(os.listdir(e1e2_dir))):  # e1e2_noisebin0.npy부터 e1e2_noisebin5.npy까지
            file_path = os.path.join(e1e2_dir, f'e1e2_noisebin{i}.npy')
            if os.path.exists(file_path):
                e1e2_files[i] = np.load(file_path)
            else:
    ...
    def _init_clean_ids(self):
        ...
        ref_file = os.listdir(self.args.data_file_dir) # 경로 재검토 필요
        temp_ids = []
        temp_ids+= [id_.strip() for id_ in ref_file]
        clean_ids = []
    ...
    def _init_psf_ids(self):
        ref_file = os.listdir(self.args.psf_dir)
        temp_ids = [id_.strip() for id_ in ref_file]
        self.psf_clean_ids = [self.args.psf_dir + id_ for id_ in temp_ids]
...
# 요주의 구간
    ...
    def __getitem__(self, idx):
        ...
        # Load PSF file
        # psf_id = random.choice(self.psf_clean_ids)
        # psf_patch = np.load(psf_id)
        ...
        if de_id < 3:
            ...
            clean_img = crop_img(np.array(np.load(clean_id).astype(np.float32)), base=16)
            ...
            degrad_img = crop_img(np.array(np.load(clean_id.replace('/gt', '/lq')).astype(np.float32)), base=16)
            degrad_patch = self.crop_transform(degrad_img)
            degrad_patch= np.array(degrad_patch)

            # Get corresponding PSF patch
            
            psf_id = clean_name.split(".")[0].replace('_0','_')
            # print('psf id :', psf_id)
            # exit()
            psf_patch = self.psf_files.get(psf_id, None)  # 안전하게 불러오기

            psf_bin, psf_index = map(int, psf_id.split('_'))
            e1e2_values = self.e1e2_files.get(psf_bin,None)
            if e1e2_values is not None:
                e1e2_patch = e1e2_values[psf_index]
                e1, e2 = e1e2_patch[0], e1e2_patch[1]
            else:
                e1, e2 = None,None
                print(f"e1e2 Patch is None for PSF ID: {psf_id}")

        else:
            if de_id == 3:
                # Rain Streak Removal
                degrad_img = crop_img(np.array(np.load(sample["clean_id"])))#, base=16)
                clean_name = self._get_gt_name(sample["clean_id"])
                clean_img = crop_img(np.array(np.load(clean_name).convert('RGB')))#, base=16)
            elif de_id == 4:
                # Dehazing with SOTS outdoor training set
                degrad_img = crop_img(np.array(np.load(sample["clean_id"]).convert('RGB')))#, base=16)
                clean_name = self._get_nonhazy_name(sample["clean_id"])
                clean_img = crop_img(np.array(np.load(clean_name).convert('RGB')))#, base=16)

            degrad_patch, clean_patch = random_augmentation(*self._crop_patch(degrad_img, clean_img))

        clean_patch = self.toTensor(clean_patch.astype(np.float32))
        degrad_patch = self.toTensor(degrad_patch.astype(np.float32))
        if psf_patch is not None:
            psf_patch = self.toTensor(psf_patch.astype(np.float32))
        else:
            # 디버깅을 위해 추가적인 로그 출력
            print(f"PSF Patch is None for ID: {psf_id}, Clean ID: {clean_id}")

        if e1 is not None and e2 is not None:
            e1e2_patch = torch.tensor([e1, e2], dtype=torch.float32)
        else:
            e1e2_patch = None

        return [clean_name, de_id], degrad_patch, clean_patch, psf_patch, e1e2_patch
...
class DenoiseTestDataset(Dataset):
    ...
    def __getitem__(self, clean_id):
        clean_img = crop_img(np.array(np.load(self.clean_ids[clean_id]).astype(np.float32)), base=16)
        clean_patch = self.crop_transform(clean_img)
        clean_patch = np.array(clean_patch)


        clean_name = self.clean_ids[clean_id].split("/")[-1]
        print('clean_id :',clean_id)
        print('clean_name :',clean_name)
        print('self.clean_ids[clean_id] :',self.clean_ids[clean_id])

        degrad_img = crop_img(np.array(np.load(self.clean_ids[clean_id].replace('/gt', '/lq')).astype(np.float32)), base=16)
        degrad_patch = self.crop_transform(degrad_img)
        degrad_patch = np.array(degrad_patch)

        psf_id = clean_name.split('.')[0].replace('_0','_')
        psf_patch = self.psf_files.get(psf_id, None)
        ...
        return [clean_name], degrad_patch, clean_patch, psf_patch, e1e2_patch
