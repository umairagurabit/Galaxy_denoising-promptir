...
def save_image_np(name, image_np, output_path="output/"):
    p = np_to_pil(image_np, output_path + "{}.png".format(name))
    # p.save(output_path + "{}.png".format(name))


def save_image_tensor(name, image_tensor, output_path="output/"):
    image_np = torch_to_np(image_tensor)
    # print(image_np.shape)
    p = np_to_pil(image_np, "{}.png".format(name))
    # p.save(output_path)
...
def np_to_pil(img_np, outpath_name, cmap='gist_ncar'):
    """
    ...
    plt.imshow(ar if img_np.shape[0] == 3 else ar, cmap=cmap)
    plt.axis('off')  # Turn off axis
    plt.savefig(outpath_name, bbox_inches='tight', pad_inches=0)  # Save the image
    plt.close()
